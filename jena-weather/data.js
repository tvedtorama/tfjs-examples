...
const REMOTE_JENA_WEATHER_CSV_PATH = "https://storage.googleapis.com/learnjs-data/jena_climate/jena_climate_2009_2016.csv"
...
export class JenaWeatherData {
	...
	async load() {
		...
		if (response != null && (response.statusCode === 200 || response.statusCode === 304)) {
			...
			console.log(`Loading data from remote path: ${REMOTE_JENA_WEATHER_CSV_PATH}`)
		}
		...
		console.log(`this.numColumnsExcludingTarget = ${this.numColumnsExcludingTarget}`)
	...
	getColumnData(columnName, includeTime, normalize, beginIndex, length, stride) {
		...
		for (let i = beginIndex; i < beginIndex + length && i < this.numRows; i += stride) {
			let value = normalize ? this.normalizedData[i][columnIndex] : this.data[i][columnIndex]
			if (includeTime) {
	 ...
	 *     `[batchSize, 1]`.
	 */
	getNextBatchFunction(shuffle, lookBack, delay, batchSize, step, minIndex, maxIndex, normalize, includeDateTime) {
        ...
        // numExamples should always be the same as batchSize, except when we run out of data
				...
				for (let j = 0; j < numExamples; ++j) {
					...
					for (let r = rowIndex - lookBack; r < rowIndex; r += step) {
						...
						const value = normalize ? this.normalizedData[r + delay][this.tempCol] : this.data[r + delay][this.tempCol]
